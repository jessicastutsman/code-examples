...
<?php foreach ($items as $i => $item) :
        ...
        if ($item['link']) {
            ...
            if ($settings['panel_link']) {
                ...
                $overlay2 = '<a class="uk-position-cover" href="' . $item->escape('link') . '"' . $link_target . '></a>';
                $overlay_hover = ' uk-overlay-hover';
            }

            if ($settings['media_overlay'] == 'icon') {
                $overlay = '<div class="uk-overlay-panel uk-overlay-background uk-overlay-' . $settings['overlay_animation'] . '"><div class="fade"><h3 class="uk-h3 uk-margin-top-remove uk-text-contrast">' . $item['title'] . '</h3><a href="'. $item->escape('link') .'"><i class="uk-icon uk-icon-chain"></i></a></div></div>' . $overlay;
            }
